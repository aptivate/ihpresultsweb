...
agency_commentary_text = {
    ...
    "2DPa" : u"En %(cur_year)s, %(one_minus_cur_val).0f%% de l’aide offerte dans le secteur de la santé a été rapporté par l’agence dans les budgets nationaux sectoriels de santé, soit une augmentation ou une diminution par rapport à %(one_minus_base_val).0f%%. Objectif-cible = réduction de 50%% de l’aide n’apparaissant pas dans le budget (dont > 85%% dans le budget).",
    "2DPb" :u"En %(cur_year)s, %(cur_val).0f%% du développement des capacités a été offert par l’agence par l’intermédiaire de programmes coordonnés, soit une augmentation, une diminution ou aucun changement par rapport à %(base_val).0f%%. Objectif-cible = 50%%.",
    "2DPc" : u"En %(cur_year)s, %(cur_val).0f%% de l’aide offerte dans le secteur de la santé a été fourni par l’agence par l’intermédiaire d’approches axées sur les programmes, soit une augmentation, une diminution ou aucun changement par rapport à %(base_val).0f%%. Objectif-cible = 66%%.",
    "3DP" : u"En %(cur_year)s, %(cur_val).0f%% de l’aide offerte dans le secteur de la santé a été fourni par l’agence par l’intermédiaire d’engagements pluriannuels, soit une augmentation, une diminution ou aucun changement par rapport à %(base_val).0f%%. Objectif-cible = 90%%.",
    "4DP" : u"En %(cur_year)s, %(cur_val).0f%% des décaissements de fonds alloués dans le secteur de la santé fournis par l’agence ont été faits conformément aux échéanciers convenus, soit une augmentation, une diminution ou aucun changement par rapport à %(base_val).0f%%. Objectif-cible = 90%%.",
    "5DPa" : u"En %(cur_year)s, %(one_minus_cur_val).0f%% de l’aide offerte dans le secteur de la santé a été fourni par l’agence par l’intermédiaire des systèmes d’approvisionnement nationaux, soit une augmentation, une diminution ou aucun changement par rapport à %(one_minus_base_val).0f%%. Objectif-cible = diminution de 33%% de l’aide offerte sans utiliser les systèmes d’approvisionnement nationaux (dont ≥ 80%% utilisent les systèmes nationaux).",
    "5DPb" : u"En %(cur_year)s, %(one_minus_cur_val).0f%% de l’aide offerte dans le secteur de la santé a été fourni par l’agence par l’intermédiaire des systèmes de gestion des finances publiques, soit une augmentation, une diminution ou aucun changement par rapport à %(one_minus_base_val).0f%%. Objectif-cible = diminution de 33%% de l’aide offerte sans utiliser les systèmes de gestion des finances publiques (dont ≥ 80 %% utilisent les systèmes nationaux).",
    "5DPc" : u"En %(cur_year)s, le nombre d’unités de mise en œuvre de projets parallèles utilisés par l’agence dans les pays étudiés était de %(cur_val)s, soit une augmentation, une diminution ou aucun changement par rapport à %(base_val)s. Object-cible = réduction de 66%% du nombre d’unités de mise en œuvre de projets parallèles.",
