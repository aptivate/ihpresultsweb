...
if (!class_exists('WPMinify')) {
  ...
  class WPMinify {
    ...
    // TBD
    //function build_minify_urls2($locations, $type) {
    //  $wpm_options = get_option($this->name);
    //  $minify_url = $this->c->get_plugin_url().'min/?g='.$type;
    //  if ($wpm_options['force_https'] && $_SERVER["HTTPS"] == "on") {
    //    $minify_url = preg_replace('/^http:\/\//', 'https://', $minify_url);
    //  }

    //  // dynamically generate groupsConfig.php if needed
    //  $fname = $this->c->get_plugin_dir().'min/groupsConfig.php';
    //  $fhandle = fopen($fname,'r');
    //  $content = fread($fhandle,filesize($fname));
    //  //$minify_url .= implode(',', $locations);
    //  // example code: 'js' => array('//js/file1.js', '//js/file2.js'),
    //  $new_code = "'js' => array('";
    //  $new_code .= '//'.implode("','//", $locations);
    //  $new_code .= "'),";
    //  $content = preg_replace(
    //    '/\/\/###WPM-GROUPS-JS-BEFORE###(.*)\/\/###WPM-GROUPS-JS-AFTER###/s',
    //    "//###WPM-GROUPS-JS-BEFORE###\n".$new_code."\n//###WPM-GROUPS-JS-AFTER###",
    //    $content);
    //  $fhandle = fopen($fname,"w");
    //  fwrite($fhandle,$content);
    //  fclose($fhandle);

    //  return array($minify_url);
    //}
