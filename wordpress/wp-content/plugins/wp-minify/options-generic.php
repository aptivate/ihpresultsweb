...
<?php
    ...
    printf('
      <p class="wpm_advanced" '.$advanced_style.'><label>%s &nbsp; <input name="wpm_options_update[debug_nominify]" type="checkbox" '.checked(true, $wpm_options['debug_nominify'], false).'/> &nbsp; (%s)</label></p>
      <p class="wpm_advanced" '.$advanced_style.'><label><a href="http://omninoggin.com/wordpress-posts/troubleshooting-wp-minify-with-firephp/">%s</a> &nbsp; <input name="wpm_options_update[debug_firephp]" type="checkbox" '.checked(true, $wpm_options['debug_firephp'], false).'/> &nbsp; (%s)</label></p>
    ...
    printf('
      ...
      <p class="wpm_advanced" '.$advanced_style.'><label>%s <strong>%s</strong><br/><textarea name="wpm_options_update[js_include]" style="width:600px" rows="5">'.attribute_escape(implode(chr(10), $wpm_options['js_include'])).'</textarea></label></p>
      ...
      <p class="wpm_advanced" '.$advanced_style.'><label>%s <strong>%s</strong><br/><textarea name="wpm_options_update[css_include]" style="width:600px" rows="5">'.attribute_escape(implode(chr(10), $wpm_options['css_include'])).'</textarea></label></p>
      ...
      __('JavaScript files to include into minify (line delimited).', $this->name),
      __('Only useful if external files is unchecked.', $this->name),
      __('JavaScript files to exclude from minify (line delimited).', $this->name),
      __('CSS files to include into minify (line delimited).', $this->name),
      __('Only useful if external files is unchecked.', $this->name),
    ...
    printf('
      ...
      ',
      __('Automatically set your Minify base per siteurl setting', $this->name)
    );
